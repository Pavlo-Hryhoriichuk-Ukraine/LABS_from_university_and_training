...
BASE = 32
FILE_NAME = f"sub_base_{BASE}.png"
...
def worker_single_subtraction(n: int, base: int) -> tuple[int, int, float]:
...
def run_optimized_subtraction_experiment() -> None:
    ...
    print("Submitting subtraction tasks to the CPU pool... Please wait.")
    print(f"{'Length (N)':<12} | {'Ops C(N)':<15} | {'Time T(N) (s)':<15}")
    print("-" * 45)

    with ProcessPoolExecutor() as executor:
        futures = [
            executor.submit(worker_single_subtraction, n, BASE)
            for n in lengths
    ...
    ax1.set_title('Operations vs Number Length (Subtraction)')
    ...
    ax2.set_title('Execution Time vs Number Length (Subtraction)')
...
if __name__ == "__main__":
    run_optimized_subtraction_experiment()